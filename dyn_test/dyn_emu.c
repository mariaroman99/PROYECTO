...
void *dyn_emu(void *vargp) {
    ...
    //Add other fields of interest of the dynamixel registers
    dyn_mem[0][0x1A] =0x03;
    dyn_mem[0][0x1B] =0x04;
    dyn_mem[0][0x1C] =0x05;
    ...
    while (true) {
        ...
        switch (fsm_state) {
            ...
            case FSM_RX__CHKSM:
                ...
                is_rx_state = false;
                break;
            case FSM_TX__HEADER_1: //canvi
                printf("\n Sending reply\n");
                tx_byte(*(((uint8_t*) &tx_header) + i++));
                break;
            case FSM_TX__HEADER_2: //canvi
                tx_byte(*(((uint8_t*) &tx_header) + i++));
                break;
            case FSM_TX__ID: //canvi
                tx_byte(tx_header.id);
                break;
            case FSM_TX__LEN: //canvi
